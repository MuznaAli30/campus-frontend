...
export default function DashStudent() {
  ...
  const isUserStudent = Cookies.get('role') === 'student';

  useEffect(() => {
    if (!isUserStudent) {
      navigate('/Login');
    }
  }, [isUserStudent, navigate]);
  ...
  <Link to='/login' onClick={handleLogout}>
    ...
    Log Out
      ...
      <div className={`flex-grow p-8 max-sm:ml-[19%] ml-32 sm:ml-10 transition-transform duration-300 ease-in-out`}>
        ...
        <h2 className="text-2xl m-auto text-center max-sm:ml-12 ml-64 max-sm:text-xs font-bold text-white bg-black mb-4 p-5">Welcom to the Student's Dashboard</h2>
