...
export default function AdminManageUsers() {
    ...
    // const Update = async (id) => {navigate(`/registration2/${id}`)}
    const Deletee = async (id) => {
    
        try {
          const response = await axios.delete(`${Api}/registration/dltStudents/${id}`);
  ...
  <Link to='/login' onClick={handleLogout}>
    ...
    Log Out
      ...
      <div className="mb-5 ml-64 max-sm:ml-12 bg-white p-5">
        ...
        {/* <button onClick={() => Update(dataa._id)}><i class="text-red-800 fa-solid fa-pen text-46"></i></button> */}
